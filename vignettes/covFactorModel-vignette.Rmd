...
my_cov_macro <- covFactorModel(assets_xts, type = "M", econ_fact = factor_xts)
cov_macro <- MacroFactor.Cov(assets, factor)
norm(cov_macro - my_cov_macro, "F")

# compare cov by BARRA Industry factor model
my_cov_BARRA <- covFactorModel(assets_xts, type = "B", stock_sector_info = sector_info)
...
my_cov_stat <- covFactorModel(assets_xts, type = "S", K = 3)
...
for (T in index_T) {
  ...
  # statistical factor model with diagonal Psi (default)
  cov_stat <- covFactorModel(X, K = num_sectors)
