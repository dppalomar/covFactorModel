...
A further refinement of the above PCA method is based on an iterative approach where each iteration performs an improved PCA (the method above corresponds to just one iteration) [see [@tsay2005analysis] for details]:
...
\end{aligned}
$$
To solve this problem, we implement a very efficient algorithm from [@khamaru2018computation], which is much faster than the one used in `stats::factanal()`.
