...
sector_info <- c(rep(1, 4),
                 ...
                 rep(3, 4))
```
Then, we use the two packages to compute the covariance matrix estimation (the comparison of execution time is ignored here for their implements are almost same) via three factor models and compare the results:
```{r, message = FALSE, warning = FALSE, cache = TRUE}
...
$$
which is proven equivalent to principal component analysis (PCA) and thus solved by:
$$
...
$$
where $\hat{\boldsymbol{\Gamma}}^{(K)} \in \mathbb{R}^{N\times K}$ with $k$-th column being the $k$-th largest eigenvector of sample covariance matrix $\hat{\mathbf{S}}$. Another choice is the refine the method with the following iterative approach [see [@tsay2005analysis] for details]:

> **Algorithm 1**    
  1. Calculate sample covariance matrix $\hat{\boldsymbol{\Sigma}}$ and eigen-decomposition (EVD): $\hat{\boldsymbol{\Gamma}}_{1} \hat{\boldsymbol{\Lambda}}_{1} \hat{\boldsymbol{\Gamma}}^{T}_{1}$    
  2. Set index $s=1$   
...
### Through further dealing with `factorModel()` results
...
Another popular covariance matrix estimation through statistical factor model is maximum likelihood estimation, which can be formulated as:
$$
...
\end{aligned}
$$
We implement an efficient algorithm [@khamaru2018computation] to slove above problem.
